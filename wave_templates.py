...
_LATE_POOL = ['fighter', 'tank', 'mage', 'assassin', 'healer', 'swarm', 'disruptor']


def get_wave_template(wave: int):
    if wave <= 5:
        base_pool = list(_EARLY_POOL)
        swarm_chance = 0.04 if wave < 4 else 0.08
    elif wave <= 14:
        base_pool = list(_MID_POOL)
        swarm_chance = 0.12
        if wave >= 10:
            base_pool.append('disruptor')
        if wave >= 12:
        ...
        swarm_chance = 0.18
        if wave >= 20:
            base_pool.extend(['tank', 'disruptor'])
